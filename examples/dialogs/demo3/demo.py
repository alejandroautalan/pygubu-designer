...
class LongRunningTask(threading.Thread):
    ...
    def run(self):
        ...
        self.app.task_cmd('task_start', dialog_id=dialog_id,
                          message=msg+' starting ...')
        
        while self.seconds > 0:
            time.sleep(1)
            self.seconds -= 1
            self.app.task_cmd('task_step', dialog_id=dialog_id,
                              message=msg+' step')
        self.app.task_cmd('task_stop', dialog_id=dialog_id,
                          message=msg+' Done.')
